...
func (client *Client)AddSkillTags(mes Сообщение) (map[string]interface{}) {
	...
	ОбновлённыйНавык , _ := ВыполнитьPgSQL(sqlStruct{
				Name:   "io_actions",
				Sql:    "UPDATE iobot.io_actions SET keywords = $1  WHERE id = $2 RETURNING *",
				Values: [][]byte{
					jsonTags,
					[]byte(АктивныеДиалоги[0]["status"].(map[string]interface{})["result"].(map[string]interface{})["io_action"].(string)),
				},
			})
	...
	if len(ОбновлённыйНавык)<1{
		...
		ОбновлённыйНавык[0]["result"]=map[string]interface{}{
			"io_action":ОбновлённыйНавык[0]["id"].(string),
		}
	}
...
func (client *Client)AddProblemDescription(mes Сообщение) (map[string]interface{}) {
	...
	ОбновлённыйНавык , _ := ВыполнитьPgSQL(sqlStruct{
				Name:   "io_actions",
				Sql:    "UPDATE iobot.io_actions SET problem = $1  WHERE id = $2 RETURNING *",
				Values: [][]byte{
					jsonПроблемы,
					[]byte(АктивныеДиалоги[0]["status"].(map[string]interface{})["result"].(map[string]interface{})["io_action"].(string)),
				},
...
func (client *Client)AllowSelfUse(mes Сообщение) (map[string]interface{}) {
	...
	ОбновлённыйНавык , _ := ВыполнитьPgSQL(sqlStruct{
				Name:   "io_actions",
				Sql:    "UPDATE iobot.io_actions SET self_use = true  WHERE id = $1 RETURNING *",
				Values: [][]byte{
					[]byte(АктивныеДиалоги[0]["status"].(map[string]interface{})["result"].(map[string]interface{})["io_action"].(string)),
				},
			})
	...
	if len(ОбновлённыйНавык)<1{
		//err = errors.New("Не удалось создать новый навык, ")
		ОбновлённыйНавык= []map[string]interface{}{0:{"error":"Не удалось разрешить самсотоятельное выполнение скрипта:"+mes.Текст}}
	} else {
		ОбновлённыйНавык[0]["result"]=map[string]interface{}{
			"io_action":ОбновлённыйНавык[0]["id"].(string),
		}
...
func (client *Client)ПолучитьАктивныйДиалог (mes Сообщение){
	...
	for _, Диалог := range АктивныеДиалоги{
		if Диалог["status"] != ""{
	...
	*/
	...
	if len(АктивныеДиалоги) >0{
		...
		if len(АктивныеДиалоги) == 1 {
			...
			СтатусАктивногоДиалога := АктивныйДиалог["status"].(map[string]interface{})
			//err := json.Unmarshal([]byte(АктивныйДиалог["status"].(string)), &СтатусАктивногоДиалога)
			//if err != nil {
			//	log.Printf("err	 %+v\n", err)
			//}

			if СтатусАктивногоДиалога["await"] == "user_answer" {
				...
				СледующееСообщение,_:=ВыполнитьPgSQL(sqlStruct{
					Name:   "io_dialogs",
					//Sql:    "SELECT * FROM iobot.io_dialogs JOIN (SELECT jsonb_array_elements(next)::integer nextId FROM iobot.io_dialogs WHERE dialog_id = $2 AND  message_id = $3) as nextSteps ON iobot.io_dialogs.id IN (nextSteps.nextId) WHERE  user_answer ? lower($1) OR user_answer is null",
					Sql:	`select next_question.*, COALESCE(varAnswers.action, next_question.action) as action from (SELECT jsonb_array_elements(next)::jsonb->> 'ответ' as answer , jsonb_array_elements(next)::jsonb->> 'далее' as next_id, jsonb_array_elements(next)::jsonb->>'action' as action FROM iobot.io_dialogs WHERE (dialog_id = $2 AND  message_id = $3)) as varAnswers JOIN iobot.io_dialogs as next_question ON next_question.id::text = varAnswers.next_id where id is not null and (case when varAnswers.answer::jsonb is not null then varAnswers.answer::jsonb ? $1 else true end )`,
					Values: [][]byte{
				...
				if Actions[АктивныйДиалог["action"].(string)] == nil{
					...
					_,_ = ВыполнитьPgSQL(sqlStruct{
						...
						Values: [][]byte{
							[]byte(АктивныйДиалог["active_id"].(string)),
							statusByte,
						},
					})
					...
					if СообщениеИО["await"] != nil{
						status = map[string]interface{}{
								"await":СообщениеИО["await"].(string),
								"result":СтатусАктивногоДиалога["result"],
							}
					} else {
						if СообщениеИО["завершено"] != nil{
							status = map[string]interface{}{
								"завершено":СообщениеИО["завершено"].(string),
								"result":СтатусАктивногоДиалога["result"],
							}
						}
					}
			...
			if  Actions[АктивныйДиалог["action"].(string)] != nil{
				...
				РезультатДейсвтия:=Actions[АктивныйДиалог["action"].(string)].(func(*Client, Сообщение)map[string]interface{})(client, mes)
				// Сделать проверку, что действие выполнилось успешно

				log.Printf("РезультатДейсвтия %+v: %+v\n", АктивныйДиалог["action"].(string), РезультатДейсвтия)
				...
				if РезультатДейсвтия["error"] != nil{
					...
					log.Printf("ОбновимСтатусСообщения %+v id %+v\n", ОбновимСтатусСообщения, АктивныйДиалог)

				}
//log.Printf("АктивныйДиалог next %+v\n",АктивныйДиалог["next"].(string) )
				if АктивныйДиалог["next"] != ""{

					//NextMessages := АктивныйДиалог["next"]
					var NextIds []json.Number
					err := json.Unmarshal([]byte(АктивныйДиалог["next"].(string)), &NextIds)
					if err != nil {
						log.Printf("err	 %+v\n", err)
					}
					log.Printf("NextIds %v \n", NextIds[0],  NextIds[0])
					...
					if len(ОтветИО)>0 && ОтветИО[0]["io"] != nil {
						...
						if РезультатДейсвтия["result"] !=nil{
							...
							} else if СообщениеИО["завершено"] != nil{
								status = map[string]interface{}{
									"завершено":"ok",
									"result":РезультатДейсвтия["result"],
								}
				...
				if client.Login == "maksimchuk@r26" {
					mes:= &Сообщение{
						Текст: "Нет обрабочтика для действия "+АктивныйДиалог["action"].(string),
						От: "io",
						Кому:client.Login,
					}
					idMes := СохранитьСообщение(*mes)
			...
			if client.Login == "maksimchuk@r26" {
				mes:= &Сообщение{
					Текст: "Более 1 активного диалога с ботом" + strconv.Itoa(len(АктивныеДиалоги)),
					От: "io",
					Кому:client.Login,
				}
				idMes := СохранитьСообщение(*mes)
